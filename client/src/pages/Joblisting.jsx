...
export default function JobListings() {
  ...
  useEffect(() => {
    const fetchJobs = async () => {
      try {
        ...
        if (filters.category !== "All Categories") queryParams.set("category", filters.category);
        if (filters.location !== "All Locations") queryParams.set("location", filters.location);
        if (filters.jobType !== "All Types") queryParams.set("jobType", filters.jobType);
        if (filters.experience !== "All Levels") queryParams.set("experience", filters.experience);
  ...
  const handleApplyFilters = (newFilters) => {
    ...
    if (newFilters.jobType !== "All Types") params.set("jobType", newFilters.jobType);
    if (newFilters.experience !== "All Levels") params.set("experience", newFilters.experience);
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50 py-12 px-4 sm:px-6 lg:px-8 pt-24">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-6 text-gray-600">
          Showing <span className="font-semibold text-gray-900">{jobs.length}</span> job
          {jobs.length !== 1 ? "s" : ""}
          {searchTerm && (
            <> for <span className="font-semibold text-blue-600">"{searchTerm}"</span></>
          )}
          {activeFiltersCount > 0 && (
            <> with <span className="font-medium text-blue-600">{activeFiltersCount}</span> filter{activeFiltersCount > 1 ? "s" : ""}</>
          )}
        </div>
        ...
        {jobs.length > 0 ? (
          <div className="space-y-6">
            {jobs.map(job => (
              <JobCard key={job._id} {...job} showSalary={true} isSaved={false} />
            ))}
          </div>
        ) : (
        ...
        {jobs.length > 0 && (
          <div className="text-center mt-12">
            <button className="px-8 py-3 text-base font-medium text-blue-600 bg-white border-2 border-blue-200 rounded-xl hover:bg-blue-50 hover:border-blue-300 focus:outline-none focus:ring-4 focus:ring-blue-500/25 transition-all duration-200 shadow-md hover:shadow-lg">
              Load More Jobs
            </button>
          </div>
        )}
